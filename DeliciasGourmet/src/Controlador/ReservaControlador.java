...
public class ReservaControlador {
	...
	// Método para realizar una reserva con múltiples mesas
	public boolean crearReservaConMultiplesMesas(Reserva reserva, List<Integer> idsMesas) {
		String[] horas = reserva.getHora().split(" - ");
		String horaInicio = horas[0];
		String horaFin = horas.length > 1 ? horas[1] : "";

		Servicio servicio = new Servicio(reserva.getFecha(), horaInicio, horaFin, 0);
		...
		PreparedStatement ps = null;
		try {
			for (Integer idMesa : idsMesas) {
				if (!verificarDisponibilidadMesa(idMesa, idServicio)) {
					System.out.println("La mesa con ID: " + idMesa + " no está disponible.");
					return false;
				}
			}
			for (Integer idMesa : idsMesas) {
				ps = connection.prepareStatement(
						"INSERT INTO Reserva (idCliente, fecha, hora, idMesa, comentario, dispocicionMesa, estado, idServicio, Temporada) "
								+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
				ps.setInt(1, reserva.getIdCliente());
				ps.setString(2, reserva.getFecha());
				ps.setString(3, reserva.getHora());
			...
			System.out.println("Reserva creada con éxito para varias mesas.");
			return true;
		} catch (SQLException e) {
			e.printStackTrace();
			System.out.println("Error al crear la reserva con múltiples mesas.");
			return false;
		} finally {
			if (ps != null) {
				try {
	...
	public boolean verificarMesaOcupada(int idMesa, String fecha, String hora) {
		...
		try {
			ps = connection
					.prepareStatement("SELECT COUNT(*) FROM Reserva WHERE idMesa = ? AND fecha = ? AND hora = ?");
			ps.setInt(1, idMesa);
	...
	public List<Reportes> obtenerHistorialDeReservas(String dateDesde, String dateHasta) {
		...
		try {
			ps = connection
					.prepareStatement("SELECT c.nombre, c.apellido, fecha, hora, m.capacidad, m.ubicacion, comentario "
	...
	public List<HistorialReserva> obtenerHistorialReserva() {
		...
		try {
			ps = connection.prepareStatement(
					"SELECT r.idReserva, c.nombre, c.apellido, r.fecha, r.hora, r.idMesa, m.capacidad, m.ubicacion, r.estado FROM Reserva r JOIN MesaPrecargada m ON r.idMesa = m.idMesa \r\n"
